		...
		function initGrid() {  
		    $('#MyDatagrid').datagrid({   //定位到Table标签，Table标签的ID是grid   
		        ...
		        columns: [[  
		             ...
		             { title: '结束时间', field: 'endTime', width: 60 ,
                             formatter : function(value){
                                 var date = new Date(value);
                                 var y = date.getFullYear();
                                 var m = date.getMonth() + 1;
                     ...
                     { title: '时段范围', field: 'periodId', width: 160 ,
                                 formatter: function(value) {
                                     var tm = $("#checkfirsttime"+value).val();
                                     return tm;
         					   }
                     },		             
					 ...
					 { title: '上传人', field: 'uploadName', width: 50 },
		             { title: '操作', field: 'uploadTime', width: 180,
                         formatter:function(value,row,index) {   
                			...
                			//+ "&nbsp;"
                			+ "<button type='button' class='btn btn-success'  style='margin:5px;' onclick='checkFirstAccess(" +index+ ")'>通过</button>"
                			//+ "&nbsp;"
                			+ "<button type='button' class='btn btn-success'  style='margin:5px;' onclick='checkFirstUnAccess(" +index+ ")'>不通过</button>";
<%-- 	                         return "<a href='#' onclick='mediaView(" +index+ ")'>预览</a>"   
		...
		function checkFirstAccess(value) {
			$.messager.confirm('提示', '初审确定通过!!', function(r){
				if(!r) return ;
				...
				$.ajax({
					type: "POST",
					url: "<%=request.getContextPath()%>/file/firstCheckFile.do",
					data: {"fileId":fiid},
					traditional: true,
					dataType : "json",
					success : function(msg){
		...
		function mediaView(value){
			...
			window.open('videoViewTable.html');
		}
...
/* 					for (var i = 0; i < rows.length; i++) { 
		...
		function doCheckSearch(){
		    ...
		    $.ajax({
			    url: '<%=request.getContextPath()%>/file/searchCheckFile.do',
...
<%-- 	<input type="hidden" id="tid" value="${ sessionScope.terminalsOne}"> --%>
	...
	<c:forEach items="${periodsCheck}" var="period" varStatus="status">
		<input type='hidden' id='checkfirsti${period.periodId}'
			value="${period.periodName}">
		<input type="hidden" id='checkfirsttime${period.periodId}'
