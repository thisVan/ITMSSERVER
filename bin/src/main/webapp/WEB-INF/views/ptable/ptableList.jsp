		...
		function initGrid() {  
		    $('#MyDatagrid').datagrid({   //定位到Table标签，Table标签的ID是grid   
		        ...
		        columns: [[  
		             ...
		             { title: '审核状态', field: 'statusId', width: 80,
		    ...
		    var roleI = "<%=session.getAttribute("rId")%>";
		    if(roleI == 1){
		    	
		    }else if(roleI == 8){
		    	document.getElementById("artificial").remove();
		    }else if(roleI == 9){
...
/* 		   value = $("#searchPname").val();
	    ...
	    function editPtableInit() {
		    ...
		    document.location = '<%=request.getContextPath()%>/ptable/goModifyPtable/' + rows[0].pid + '.do';
		}
	     ...
	     function artificialGenerated(){
	    	 ...
	    	 $.messager.confirm('提示框', '你确定要重新生成播表?', function(r) {
		        if(!r) return;
		    	 $.ajax({
						type : "POST",
						url : "<%=request.getContextPath()%>/ptable/resetPtable.do",
						//data : formData,
						processData : false,
						contentType : false,
						dataType : "json",
						success : function(date) {
							...
							if(value=="true") {
								$.messager.alert('提示', '播表重新生成成功');
							}else{
								$.messager.alert('提示', '播表重新生成失败');
							}
						}
			     });
	          })
...
<!-- 	     	<span>播表名称:</span>
		...
		<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-search" plain="true" onclick="editPtableInit()">播表详情</a>  
 	    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-add" id="insert" plain="true" onclick="insertPtable()">插播</a>
		<!-- <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-remove" plain="true" onclick="addPtableInit()">新建播表</a> -->
		 <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-sum" id="artificial" plain="true" onclick="artificialGenerated()">手动生成播表</a>
