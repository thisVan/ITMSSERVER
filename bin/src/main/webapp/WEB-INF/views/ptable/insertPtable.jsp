...
/* 		  for(var i = 0; i < fileIds.length; i++) {
		...
		function growItemContent(row) {
             var content = '<tr id = "fileId-' +  row.fileId + '-' + row.materialName +'" >' + 
		                     '<td>' + row.materialName + '</td>' + 
		                     '<td>' + row.duration + '</td>' + 
		                     '<td>' + easyuiDateFormat(row.uploadTime) + '</td>' + 
		                     '<td><button type="button" class="btn btn-danger" style="margin:5px;" onclick="delSelectFile(' + row.fileId + ')">取消</button></td>' + 
		                  '</tr>';
		     return content;
		}
		...
		<div class="panel panel-default">
			<div class="panel-heading">
				<h5 class="sub-header">
					...
					<span style="margin-left: 2px;">终端已有插播信息:&nbsp;&nbsp;&nbsp;&nbsp;终端序列号:
...
<%--  					   	<c:forEach items="${insertTables}" var="table" varStatus="status">
  ...
  <legend>同时绑定多个</legend>
				...
				&nbsp;&nbsp;<label id="frequ" style="width:40px;"></label>
			</div>
		</form>
		<br />
