...
//		        System.out.println();
	...
	//生成同时段多播表
	public static void writeSqlTable(int terminalId, int periodId, List<Ad> ad, String uid, int len) throws ParseException {
		Date date = new Date();
		...
		if(screenTime <= 0) screenTime = 24*60*60*1000;
		...
		public void deleteNewPlayTable(int periodId, int start, int end) {
			try {
				...
				if(!conn.isClosed()) {
					...
					//Date nextDate=cal.getTime();
					//java.sql.Date sqlDate = new java.sql.Date(nextDate.getTime());
					//String sql = "select pid from play_table where date_format(play_date, '%Y-%m-%d')='" + sqlDate + "' and deleted = 0";
					for(int i = start; i <= end; i++) {
						...
						Date nextDate=cal.getTime();
						java.sql.Date sqlDate = new java.sql.Date(nextDate.getTime());
						String sql = "select pid from play_table where period_id = " + periodId + " and date_format(play_date, '%Y-%m-%d')='" + sqlDate + "' and deleted = 0";
...
//					String sql = "select pid from play_table where period_id = " + periodId + " and deleted = 0";
					//System.out.println(sql);
//					Statement statement = conn.createStatement();
//					ResultSet rs = statement.executeQuery(sql);
//					List<String> list = new ArrayList<String>();
	...
	public static int getReadSqlTable(int terminalId, int periodId) throws ParseException{
		...
		try {
			...
			String sql = "select pid from play_table where deleted = 0 and terminal_id = "
					+ terminalId + " and  period_id = " + periodId;
			System.out.println(sql);
...
//		SimpleDateFormat f = new SimpleDateFormat("YYYY-MM-DD HH:MM:SS");
	...
	public static int getScreenTime(int periodId) {
		...
		try {
			...
			Statement statement = conn.createStatement();
			ResultSet rs = null;
			String sql = "select start_interval, end_interval from period where deleted = 0 and " + " period_id = " + periodId;
			System.out.println(sql);
			rs = statement.executeQuery(sql);
			while (rs.next()) {
