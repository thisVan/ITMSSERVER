...
<input type="hidden" id="mid" value=""/>
  ...
  <div class="layui-form-item">
    ...
    <div class="layui-input-inline">
      <input type="text" id="opacity" name="opacity" placeholder="请输入透明度" autocomplete="off" style="width:250px;" class="layui-input input-text"/>
    </div>
  </div>
...
layui.use(['layer','form','laydate'], function(){
   ...
   function updateMaterial(){
	   ...
	   if(isNaN(opacity)){
		   layer.msg('输入的透明度数不符!!!',{icon:5,time:2000});
			return ;
		}
	   if(parseInt(opacity)==opacity){
	    	
	    }else{
	    	layer.msg('输入的透明度数不是整数!!!',{icon:5,time:2000});
			return ;
	    }
	   ...
	   if(periodName == "" || testDate == "" || opacity == ""){
		   ...
		   $.ajax({
				type: "POST",
				url: "<%=request.getContextPath()%>/material/editMarquee.do",
				data:formData,
				async: false,
				processData : false,
				contentType : false,
