		...
		function initGrid() {  
		    $('#MyDatagrid').datagrid({   //定位到Table标签，Table标签的ID是grid   
		        ...
		        columns: [[  
                     ...
                     { title: '错误原因', field: 'errorReason', width: 200},
                     { title: '记录日期', field: 'recordDate', width: 100,
		...
		function delErrorLog(){
		    ...
		    $.messager.confirm('提示框', '你确定要删除选中的日志?', function(r) {
			    ...
			    for(var i = 0; i < rows.length; i++) {
			      ids.push(rows[i].errorLogId);
			    }
			    
			    
			     $.ajax({
				    url: '<%=request.getContextPath()%>/logController/delErrorLog.do',
				    data:{"ids": ids}, 
				    type: "post",
				    traditional: true,  
				    dataType : "json"
	 ...
	 <div region="center" border="false" style="padding:5px;">
	  ...
	  <div id="tb"> <!-- datagrid 的工具栏  start -->
	    ...
	    <form id="searchForm" style="padding:5px;">
		    ...
		    <span>错误原因:</span>
			<input id="searchErrorReason" name="errorReason" type="text" value="">
           	<span>记录日期:</span>
		...
		<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-remove" plain="true" onclick="delErrorLog()">删除记录</a>
