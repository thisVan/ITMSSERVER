		...
		function initGrid() {  
		    $('#MyDatagrid').datagrid({   //定位到Table标签，Table标签的ID是grid   
		        ...
		        columns: [[  
		             ...
		             { title: '播放的视频名称', field: 'materialName', width: 150},  
		             { title: '开始播放时间', field: 'playStartTime', width: 100,
                         formatter:function(value,row,index) {   
	                         return easyuiDateFormat(value);  
                         }
                     },
                     { title: '结束播放时间', field: 'playEndTime', width: 100,
                         formatter:function(value,row,index) {   
	                         return easyuiDateFormat(value);  
                         }
                     },
                     { title: '所属的播表名称', field: 'pname', width: 150}
		        ]],  
                toolbar: '#tb'
		    });
		...
		function delPlayLog(){
		    ...
		    $.messager.confirm('提示框', '你确定要删除选中的日志?', function(r) {
			    ...
			    for(var i = 0; i < rows.length; i++) {
			      ids.push(rows[i].playLogId);
			    }
			    
			    
			     $.ajax({
				    url: '<%=request.getContextPath()%>/logController/delPlayLog.do',
				    data:{"ids": ids}, 
				    type: "post",
				    traditional: true,  
				    dataType : "json"
	 ...
	 <div region="center" border="false" style="padding:5px;">
	  ...
	  <div id="tb"> <!-- datagrid 的工具栏  start -->
	    ...
	    <form id="searchForm" style="padding:5px;">
			...
			<span>播放的视频名称:</span>
			<input id="searchMaterialName" name="materialName" type="text" value="">
			<span>所属的播表名称:</span>
			<input id="searchPname" name="pname" type="text" value="">
			<br/>
			
			<span>开始播放时间:</span>
            <input id="searchPlayStartTime" name="playStartTime" type="text" value="" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',lang:'zh-cn'})">
           	<span>结束播放时间:</span>
            <input id="searchPlayEndTime" name="playEndTime" type="text" value="" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',lang:'zh-cn'})">
		...
		<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-remove" plain="true" onclick="delPlayLog()">删除记录</a>
