		...
		function initGrid() {  
		    $('#MyDatagrid').datagrid({   //定位到Table标签，Table标签的ID是grid   
		        ...
		        columns: [[  
		             ...
		             { title: '播表名', field: 'ptableName', width: 170},
		             //{ title: '终端ID', field: 'terminalId', width: 40},
		             { title: '时段范围', field: 'periodId', width: 170 ,
		             ...
		             { title: '审核状态', field: 'statusId', width: 60,
                     ...
                     { title: '可播时长', field: 'allTime', width: 70},
                     { title: '占屏比', field: 'screenRate', width: 70},
		             { title: '创建人', field: 'userId', width: 60 ,
	                        formatter: function(value) {
		             ...
		             { title: '操作', field: 'deleted', width: 60,
                         formatter:function(value,row,index) {
...
/* 		   value = $("#searchPname").val();
	    ...
	    function editPtableInit(ptableId) {
		...
		function gradeChange(){
			var stat = $("#searchPlayStatusSelect").val();
			if(stat == 1){
				initGrid();
				doSearch();
			}else if(stat == 4){
...
<!--   	    <form id="searchForm" style="padding:5px;">
			...
			<span>审核状态:</span>
			<select id="searchPlayStatusSelect" name="fileStatus" onchange="gradeChange()">
